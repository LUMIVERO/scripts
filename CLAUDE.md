...
- **Side-scroller animation:** on an interactive TTY the checks are framed as a little flying game. The developer 🧑‍💻 holds a fixed spot on the left and flies up and down through open sky; below the flight rows sit a scrolling tree line (trees with sky showing through the gaps), a green grass band, and a brown earth band at the bottom. The tree line scrolls left every frame (`SCROLL`, sampled from the repeating `GROUND_SCENERY` strip) — so the stationary developer reads as flying rightward. Each check arrives from the right as a beast (a different dangerous emoji per check) flying left at that same one-cell-per-frame speed; the developer lines up its row and blasts it, the beast turns into a fruit that scrolls back and is digested as the developer flies over it. A failing check lets the beast reach the developer, who plummets and crashes (💥) into the tree line, with a cheerful "Respawn quest" fix hint below. Results accumulate beneath the field. It's a full-screen redraw per frame, so it runs **only** when stdout is a TTY and degrades to the plain line-by-line output otherwise (CI, pipes). `SETUP_NO_GAME=1` opts out. Each check plays a self-contained scene (beast/fruit picked from the `BEASTS`/`FRUITS` lists by check number), so the animation scales to the check count automatically — there is no per-check counter to maintain.
