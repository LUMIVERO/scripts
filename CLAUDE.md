...
- **Fixes are auto-accepted by default under `curl … | sh`.** `resolve_assume_yes` (run once at the top of `main`) sets `ASSUME_YES=1` — so `confirm` returns yes without prompting — whenever the script was *piped* into a shell (detected via `running_piped`: `$0`'s basename is a shell name, because the shell read the script from stdin). A *direct* run (`sh <app>_setup.sh`, `./…`) prompts per fix instead, unless `--yes`/`-y` is passed or `SETUP_ASSUME_YES=1` is set in the environment. The interactive branch menu in `select_repo_branch`, by contrast, is **not** a fix and still shows under a piped run — the developer is at a terminal (the prompt reads from `/dev/tty`) and choosing a branch right after a fresh clone is a genuine selection, not something to auto-accept. It is skipped only when assume-yes was requested *explicitly* (`--yes`/`-y` or `SETUP_ASSUME_YES=1`, recorded as `ASSUME_YES_EXPLICIT` — an outright "no interaction") or when there is no `/dev/tty` (headless run).
